...
def declare_logic():
    ...
    # Ensure that customer balance does not exceed their credit limit.
    ...
    # Calculate the amount for each order detail line item.
    Rule.formula(derive=OrderDetail.amount, as_expression=lambda row: row.quantity * row.item.price)

    # Count the number of service activities a student is involved in.
    Rule.count(derive=Student.service_activity_count, as_count_of=StudentActivity, where=lambda row: row.activity.is_service_activity)
