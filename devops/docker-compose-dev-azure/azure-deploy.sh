...
resourcename="potterystore"  # lower case, only
resourcegroup="potterystore_rg"
...
# git clone https://github.com/apilogicserver/potterystore.git
# cd potterystore
...
az group create --name potterystore_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group potterystore_rg --sku S1 --is-linux
az container create --resource-group potterystore_rg --name potterystore --image apilogicserver/potterystore:latest --dns-name-label potterystore --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//potterystore.westus.azurecontainer.io:5656'
